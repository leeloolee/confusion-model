...
class TextEncoder(keras.Model): ## TODO: fix embedding
    def __init__(
        self, max_length, vocab_size=49408, name=None, download_weights=False
    ):
...
#         x = CLIPEmbedding(vocab_size, 512, max_length)([tokens, positions])
#         for _ in range(23):
#             x = CLIPEncoderLayer(512, 16, activation=tf.nn.gelu)(x)
#         embedded = keras.layers.LayerNormalization(epsilon=1e-5)(x)
#         super().__init__([tokens, positions], embedded, name=name)
#
#         if download_weights:
...
class CLIPAttention(keras.layers.Layer):
    def __init__(self, embed_dim=232, num_heads=12, causal=True, **kwargs):
        super().__init__(**kwargs)
