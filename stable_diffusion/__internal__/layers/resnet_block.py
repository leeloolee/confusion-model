...
class ResnetBlock(keras.layers.Layer):
    def __init__(self, output_dim, **kwargs):
        ...
        self.conv1 = PaddedConv1D(output_dim, kernel_size=3, padding=(0,1))
        self.norm2 = keras.layers.GroupNormalization(epsilon=1e-5)
        self.conv2 = PaddedConv1D(output_dim, kernel_size=3, padding=(0,1))

    def build(self, input_shape):
        if input_shape[-1] != self.output_dim:
            self.residual_projection = PaddedConv1D(self.output_dim, 1)
        else:
