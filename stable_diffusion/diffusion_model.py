...
class DiffusionModel(keras.Model):
    def __init__(
        ...
        context = keras.layers.Input((1536,))#max_text_length, obs_act*horizon)) #
        # context = keras.layers.Reshape((-1, 2, 768))(context)
        # context2 = keras.layers.Reshape((-1, obs_act))(context)

        t_embed_input = keras.layers.Input((320,))

        latent = keras.layers.Input((obs_act, horizon // 8, 1))
        ...
        # Downsampling flow
        outputs = []
        x = PaddedConv1D(320, kernel_size=3, padding=(0,1))(latent)
        outputs.append(x)
        for _ in range(2):
            x = ResBlock(320)([x, t_emb])
            x = SpatialTransformer(8, 40, fully_connected=False)([x, context])
            outputs.append(x)
        x = PaddedConv1D(320, kernel_size=3, strides=2, padding=(0,1))(x)  # Downsample 2x
        outputs.append(x)
        for _ in range(2):
            x = ResBlock(640)([x, t_emb])
            x = SpatialTransformer(8, 80, fully_connected=False)([x, context])
            outputs.append(x)
        x = PaddedConv1D(640, kernel_size=3, strides=2, padding=(0,1))(x)  # Downsample 2x
        outputs.append(x)
        for _ in range(2):
            x = ResBlock(1280)([x, t_emb])
            x = SpatialTransformer(8, 160, fully_connected=False)([x, context])
            outputs.append(x)
        x = PaddedConv1D(1280, kernel_size=3, strides=2, padding=(0,1))(x)  # Downsample 2x
        outputs.append(x)
        for _ in range(2):
            x = ResBlock(1280)([x, t_emb])
            outputs.append(x)
        ...
        output = PaddedConv1D(1, kernel_size=3, padding=(0,1))(x)
...
#         x = keras.layers.GroupNormalization(epsilon=1e-5)(x)
#         x = keras.layers.Activation("swish")(x)
#         output = PaddedConv1D(4, kernel_size=3, padding=(0,1))(x)
#
#         super().__init__([latent, t_embed_input, context], output, name=name)
#
#         if download_weights:
...
class ResBlock(keras.layers.Layer):
    def __init__(self, output_dim, **kwargs):
        ...
        self.entry_flow = [
            ...
            PaddedConv1D(output_dim, kernel_size=3, padding=(0,1)),
        ]
        self.embedding_flow = [
            keras.layers.Activation("swish"),
            keras.layers.Dense(output_dim),
        ]
        self.exit_flow = [ # TODO Error
            keras.layers.GroupNormalization(epsilon=1e-5),
            keras.layers.Activation("swish"),
            PaddedConv1D(output_dim, kernel_size=3, padding=(0,1)),
        ]
    ...
    def build(self, input_shape):
        if input_shape[0][-1] != self.output_dim:
            self.residual_projection = PaddedConv1D(self.output_dim, 1, padding = False)
        else:
...
class SpatialTransformer(keras.layers.Layer):
    def __init__(self, num_heads, head_size, fully_connected=False, **kwargs):
        ...
        if fully_connected:
            ...
            self.proj1 = PaddedConv1D(num_heads * head_size, 1,  padding = False)
        ...
        if fully_connected:
            ...
            self.proj2 = PaddedConv1D(channels, 1, padding = False)
...
class Upsample(keras.layers.Layer):
    def __init__(self, channels, **kwargs):
        super().__init__(**kwargs)
        self.ups = keras.layers.UpSampling2D((1,2)) ## 2=>(1,2)
        self.conv = PaddedConv1D(channels, kernel_size=3, padding=(0, 1))
