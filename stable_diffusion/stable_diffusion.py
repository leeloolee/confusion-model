...
class StableDiffusionBase:
    ...
    def __init__(
        ...
        obs_act = obs_act
        horizon = round(horizon / 128) * 128 ## 고치자 치
    ...
    def encode_text(self, prompt):
        ...
        context = openai.Embedding.create(model=self.openai_model, input=prompt)["data"][0]["embedding"]
        ...
        # )
        return np.array(context) # [np.newaxis,:]
    ...
    def generate_image(
        ...
        context = tf.reshape(context, (batch_size, -1, 1))
        ...
        unconditional_context = tf.reshape(unconditional_context, (batch_size, -1, 1))
        ...
        return np.clip(decoded, -1, 1).astype("float32")
    ...
    def inpaint(
        ...
        encoded_text = tf.reshape(encoded_text, (batch_size, -1, 1)) ### 가해준거ㄱ주해추


        # 기존 Trajectory
        image = tf.squeeze(image)
        image = tf.cast(image, dtype=tf.float32) / 2.0 + 1.0
        image = tf.expand_dims(image, axis=0)
        known_x0 = self.image_encoder(image)
        if image.shape.rank == 3: # 3->2
            known_x0 = tf.repeat(known_x0, batch_size, axis=0)

        mask = tf.expand_dims(mask, axis=-1)
        mask = tf.cast(
            tf.nn.max_pool2d(mask, ksize=8, strides=(1,8), padding="SAME"),
            dtype=tf.float32,
        )
    ...
    def _get_embedding(self, text:str, model:str):
        """Returns embedding for the given text."""
        return openai.Embedding.create(model=model, input=text, max_token=1536)["data"][0]["embedding"] # max token is defined by openai.Embedding

    def _get_unconditional_context(self):
        # unconditional_tokens = tf.convert_to_tensor(
        #     [_DITIONAL_TOKENSUNCON], dtype=tf.int32
        # )
        # unconditional_context = self.text_encoder.predict_on_batch(
        #     [unconditional_tokens, self._get_pos_ids()]
        # )
        unconditional_context = ""
        unconditional_context = np.array(self._get_embedding(unconditional_context, self.openai_model)) #[np.newaxis, :]
    ...
    def image_encoder(self):
        ...
        if self._image_encoder is None:
            self._image_encoder = ImageEncoder(self.obs_act, self.horizon)
            if self.jit_compile:
    ...
    def decoder(self):
        ...
        if self._decoder is None:
            self._decoder = Decoder(self.obs_act, self.horizon)
            if self.jit_compile:
    ...
    def _get_initial_diffusion_noise(self, batch_size, seed):
        if seed is not None:
            return tf.random.stateless_normal(
                (batch_size, self.obs_act, self.horizon // 8, 1),
                seed=[seed, seed],
            )
        else:
...
class StableDiffusion(StableDiffusionBase):
    ...
    def __init__(
        ...
        super().__init__(obs_act, horizon, jit_compile)
        print(
    ...
    def diffusion_model(self):
        ...
        if self._diffusion_model is None:
            self._diffusion_model = DiffusionModel(
                self.obs_act, self.horizon, MAX_PROMPT_LENGTH
            )
            if self.jit_compile:
